...
class PromptBuilder:
    ...
    KEYWORDS=(("audit","audit"),("analyze","analyze"),("analyse","analyze"),("repair","repair"),("fix","fix"),("refactor","refactor"),("implement","implement"),("create","create"),("review","review"))
    def resolve_profile(self,objective:str)->str:
        t=objective.lower()
        for k,p in self.KEYWORDS:
            if k in t:return p
        return "default"
    def build(self,*,system_prompt:str,objective:str,repository_context:str,additional_context:str="",context_budget:int|None=None):
...
{additional_context}
'''
        return [Message(role="system",content=system_prompt),Message(role="user",content=user.strip())]
    ...
    def _trim(self,text:str,budget:int)->str:
        if self.fits_budget(text,budget):return text
